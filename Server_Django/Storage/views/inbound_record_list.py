...
from Core.models import InboundRecord
from User.views.user_info import get_user_from_token


class InboundRecordListView(View):
    """
    获取入库记录列表视图
    """
    ...
    def get(self, request):
        try:
            ...
            # 构建查询条件
            queryset = InboundRecord.objects.select_related('commodity').all()
            ...
            # 按入库时间倒序排列
            queryset = queryset.order_by('-inbound_time')
            ...
            for record in page_obj:
                record_data = {
                    ...
                    'inboundName': record.inbound_name,
                    'inboundJobNumber': record.inbound_job_number,
                    'inboundTime': record.inbound_time.strftime('%Y-%m-%d %H:%M:%S')
                }
